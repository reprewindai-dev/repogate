...
export default function App() {
  ...
  const startGovernedReview = () => {
    ...
    // Assign randomized Run Ref and Governor Agent ID
    const sampleRunId = 'run_' + Math.random().toString(36).substr(2, 6).toUpperCase();
    const sampleAgentId = 'agent_' + Math.floor(Math.random() * 100).toString().padStart(3, '0');
    
    // Generate targets
    const { paths, isTruncated } = generateFilesForRepo(repoUrl);
    const mockFindings = scanFilesForFindings(paths, sampleRunId);
    const topRisk = determineOverallRisk(mockFindings);
